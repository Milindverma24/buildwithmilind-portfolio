...
const LOGO_HORIZONTAL = readLogoB64('MILIND VERMA.png');
const LOGO_ICON = readLogoB64('MILIND VERMA.png');
...
<head>
  ...
  <title>New Message — Milind Verma</title>
  <style>
          ...
          <tr>
            <td style="padding-top: 20px;">
              <p style="font-size:10px; letter-spacing:0.12em; color:rgba(255,255,255,0.12); line-height:1.8;">
                This message was submitted via the contact form at milindverma24.github.io<br/>
                © 2026 Milind Verma · All rights reserved
              </p>
            </td>
          </tr>
...
export async function POST(req: NextRequest) {
  try {
    ...
    const hasRealKey = process.env.RESEND_API_KEY && 
    ...
    if (!hasRealKey) {
      ...
      console.log(`Subject: New message from ${name} — Portfolio`);
      console.log(`Message:\n${message}`);
      console.log('==========================================================');

      // Also append to a local file for easy developer viewing
      try {
        const logDir = path.join(process.cwd(), 'logs');
        if (!fs.existsSync(logDir)) {
          fs.mkdirSync(logDir);
        }
        const logPath = path.join(logDir, 'contact_messages.txt');
        const logEntry = `[${new Date().toISOString()}] FROM: ${name} <${email}>\nMESSAGE:\n${message}\n--------------------------------------------------\n\n`;
        fs.appendFileSync(logPath, logEntry, 'utf8');
      } catch (logErr) {
        console.error('Failed to write local contact log:', logErr);
      }
    ...
    const { error } = await resend.emails.send({
      ...
      to: 'anshverma24112005@gmail.com',
      replyTo: email,
